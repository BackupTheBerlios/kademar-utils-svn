...
#  HelioxStarterScreenlet (c) Adonay Sanz Alsina 2009
...
# i18n
import gettext
gettext.install("HelioxStarterScreenlet", os.path.abspath(os.path.dirname(sys.argv[0])) + "/locale")
...
""" MPD Screenlet """
class HelioxStarterScreenlet(screenlets.Screenlet):
	# Info
	__name__ = 'Accessibility - Heliox Starter'
	__version__ = '0.4'
	__author__ = 'Adonay Sanz Alsina -adonay@kademar.org- using code of Krzysztof Magusiak'
	__desc__ = 'Accessibility Heliox applet to start applications. Developed for Heliox Linux.'	
	
	
	# settings
	global __scrread__
	screenread_buttons = __scrread__ = True
	mypath = sys.argv[0][:sys.argv[0].find('HelioxStarterScreenlet.py')].strip()
	...
	def __init__(self, **keyword_args):
		...
		self.add_options_group('Heliox Starter', 'Specific options')

		self.add_option(BoolOption('Heliox Starter', 'screenread_buttons', self.screenread_buttons, 'Screenreader', 'Enable reading content button with a screenreader'))
		# Defautl values
...
if __name__ == '__main__':
	...
	screenlets.session.create_session(HelioxStarterScreenlet)
