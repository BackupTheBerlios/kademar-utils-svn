...
class instalador(QDialog):
    def __init__(self):
        ...
        #  VARIABLES
        ######
        global led_order, page_order, icona_verda, icona_taronja, icona_vermella, icona_blava, band_cat, band_esp, band_eng, copiant, fs_detector, grephalinfo, target, inicial, icona_hd, icona_partition, mkfilesystems, filesystems, varcopiaacabada, pathinstaller, posicioprogress
...
#####
    def fillaccount(self, char):
        ...
        self.ui.t_user_account.setText(nom)  #Defineix el nom del compte
...
#Funcio a executar al acabar
    ...
    #########3   3#######
        ...
        #############################
            ...
            ###################################
            cuenta=self.ui.t_user_account.text()
            ...
            uidslliures=[]
            uidsocupats=[]
            for l in open(target+'/etc/passwd'):
                campos = l.split(':')
            if (int(campos[2])>999) and (int(campos[2])<60000):
                llista.append(int(campos[2]))
                llista.append(campos[0])
                uids.append(llista)
                llista=[]
                uidsocupats=[]
            for i in range(len(uids)):
                uidsocupats.append(uids[i][0])
            for i in range(1000,1010):
                if i not in uidsocupats:
                    uidslliures.append(i)
...
#                     gecos=cuenta
                    home='/home/'+cuenta
                    bash='/bin/bash'
                    if not path.exists(target+home):
                    ...
                    f=open(target+'/var/tmp/usu.txt','w')
                    f.writelines(linia)
                    f.close()
                    system('chroot '+target+' /usr/sbin/newusers /var/tmp/usu.txt')
                    system('rm -f '+target+'/var/tmp/usu.txt')
                    ...
                    if crea_home=="creahome_si":
                        system('mkdir -p "'+str(target)+str(home)+'"')
                        system('cp -a '+str(inicial)+'/home/kademar/* '+str(target)+str(home)+'\n')
                        system('cp -a '+str(inicial)+'/home/kademar/.??* '+str(target)+str(home)+'\n')

            system("sh scripts/install-usuaris")
            #borrem el home kademar si l'usuari no es diu kademar
            if not self.ui.t_user_account.text() == "kademar":
                system('rm -fr '+str(target)+'/home/kademar')
	...
	################
        ...
        ################
            ...
            system("tar cfz "+target+"/usr/share/kademar/install.log.tar.gz /tmp/kademar* /tmp/particions* /tmp/instalador-environment /tmp/particio_swap /var/xserver /var/xsession* /var/es /var/en /var/ca /etc/environment /etc/kademar-release /usr/share/kademar/config* /var/kademar* "+target+"/boot/grub/menu.lst 2>/dev/null")
