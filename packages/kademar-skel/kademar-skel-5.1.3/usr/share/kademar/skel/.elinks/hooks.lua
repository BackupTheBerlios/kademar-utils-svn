...
dofile_if_fileexists ("/etc/elinks/config.lua")
...
--    writeto (tmp) write (current_document ()) writeto ()
    if doc then writeto (tmp) write (doc) writeto () end
    table.insert (tmp_files, tmp)
    return "run", "exec mutt -a "..tmp
--    return os.execute("mutt -a "..tmp)
end

function lpr ()
   local tmp = tmpname ()
   local doc = current_document (79)
   if doc then writeto (tmp) write (doc) writeto () end
   table.insert (tmp_files, tmp)
   return "run", "exec /usr/bin/adriane-print "..tmp
-- Hängt sich auf. :-(
-- return os.execute("/usr/local/bin/adriane-print "..tmp)
end
